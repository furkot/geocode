...
function getStatus(err, response) {
  if (err) {
    if (err.status === 401 || (err.status === 429 && response.error === 'Rate Limited Day')) {
...
function getUrl(url, key, op, query) {
  ...
  q.push(`key=${key}`);
  if (query.partial) {
    url = url.replace('us1.locationiq.com', DEFAULT_URL).replace('eu1.locationiq.com', DEFAULT_URL);
  }
...
function map(f) {
  ...
  const place = {
    ll: [Number.parseFloat(f.lon), Number.parseFloat(f.lat)],
  ...
  if (f.display_name) {
    ...
    if (place.place === a.house_number || place.place === place.street || place.place === (a.road || a.pedestrian)) {
      delete place.place;
    }
  }
...
function processResponse(places, _query, result) {
...
function init(options) {
  options = util.defaults(options, {
    forward: true,
    reverse: true,
    url: getUrl.bind(undefined, options.locationiq_url || `https://${DEFAULT_URL}/v1/`, options.locationiq_key),
    status: getStatus,
    prepareRequest,
    processResponse
  });
