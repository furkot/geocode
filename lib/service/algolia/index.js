...
function processResponse(response) {
  ...
  return {
    places: response.hits.map(function (result) {
...
function init(options) {
  ...
  function prepareRequest(op, query) {
    ...
    if (options.algolia_key) {
      req['x-algolia-api-key'] = options.algolia_key;
      req['x-algolia-application-id'] = options.algolia_app_id;
    }
    return req;
  }
  ...
  options = util.defaults(options, {
    forward: true,
    post: true,
    url: options.algolia_url || 'https://places-dsn.algolia.net1/places/query',
    status: getStatus,
