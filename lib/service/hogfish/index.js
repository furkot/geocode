...
function prepareRequest(types, op, query) {
  if (op === 'forward') {
    query.type = query.type ?? ['place', 'address'].find(type => Object.hasOwn(query, type));
  }
...
function isCloser(result, place) {
  const dist = (result.ll[0] - place.ll[0]) ** 2 + (result.ll[1] - place.ll[1]) ** 2;
...
function findPlace(result, place) {
  if ((!result.place || (place.name && matchNames(place.name, result.place))) && isCloser(result, place)) {
