...
function furkotGeocode(options) {
  ...
  function geocode(query, fn) {
    var timeoutId, queryId, op, aborted, max;
    ...
    max = query.max || options.max;
    query.max = query.preference ? Math.min(4 * max * query.preference.length, 100) : max;
    strategy(operations[op], queryId, query, {}, function (err, queryId, query, result) {
      ...
      if (result && result.places) {
        if (query.address || query.place) {
          result.places = result.places.filter(query.place ? isPlace : isAddress);
        }
        if (query.preference) {
          result.places = group(query.preference, result.places);
        }
        if (max && result.places.length > max) {
          result.places.length = max;
        }
