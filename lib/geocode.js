  ...
  function geocode(query, fn) {
    ...
    strategy(operations[op], queryId, query, {}, function (err, queryId, query, result) {
      ...
      if (result && result.places) {
        result.places.forEach(place => place.address = place.address || stringify(place));
        const tokens = query.place || query.address;
        if (tokens) {
          result.places = result.places.filter(includesTokens, {
            key: query.place ? 'place' : 'address',
            tokens
          });
        }
        if (query.max && result.places.length > query.max) {
...
function findToken(t) {
  const { str } = this;
  return str.includes(t);
}

function includesTokens(place) {
  const {
    key,
    tokens
  } = this;
  return place[key] && tokens.toLowerCase().split(' ').some(findToken, { str: place[key].toLowerCase() });
}
